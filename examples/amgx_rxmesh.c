...
int main(int argc, char **argv)
{
    ...
    int M, N, nz;
    void *b_h[3]; // pointers to the 3 different vectors (can make this a parameter)
    void *x_h[3]; // pointers to the 3 different vectors
    ...
    {
        // uploads input matrix into a, 
        // creates b, x matrices as [1, 1, 1...]^T and  [0, 0, 0...]^T respectively
    ...
    {
        FILE *f = fopen(argv[pidx + 1], "r");
        MM_typecode matcode;

        int B_M, B_N, B_nz;
        mm_read_banner(f, &matcode);
        mm_read_mtx_array_size(f, &B_M, &B_N);
        printf("Number of rows: %d, Number of cols: %d\n", B_M, B_N);

        for (int i = 0; i < B_N; i++)
        {
            b_h[i] = malloc(B_M * sizeof_v_val);
            for (int j = 0; j < B_M; j++)
            {
                if (sizeof_v_val == sizeof(double))
                {
                    double *b_h_d = (double *)(b_h[i]);
                    if (fscanf(f, "%lf", &b_h_d[j]) != 1)
                    {
                        fprintf(stderr, "Error reading b vector values.\n");
                        free(b_h[i]);
                        fclose(f);
                        exit(1);
                    }
    ...
    // populates x matrix
    if ((pidx = findParamIndex(argv, argc, "-X")) != -1)
    {
        FILE *f = fopen(argv[pidx + 1], "r");
        MM_typecode matcode;
        int X_M, X_N, nz;
        ...
        for (int i = 0; i < X_N; i++)
        {
            x_h[i] = malloc(X_M * sizeof_v_val);

            for (int j = 0; j < X_M; j++)
            {
                if (sizeof_v_val == sizeof(double))
                {
                    double *x_h_d = (double *)(x_h[i]);
                    if (fscanf(f, "%lf", &x_h_d[j]) != 1)
                    {
                        fprintf(stderr, "Error reading x vector values.\n");
                        free(x_h[i]);
                        fclose(f);
                        exit(1);
                    }
    ...
    if (pidx != -1)
    {
        ...
        x_h_repeat = malloc(n * block_dimx * sizeof_v_val);
        AMGX_vector_download(x, x_h_repeat);
    ...
    for (int r = 0; r < nrepeats; ++r)
    {
        for (int i = 0; i < N; i++) {
            ...
            AMGX_vector_upload(b, M, 1, b_h[i]);
            free(b_h[i]);
             /* solver setup */
            ...
            result_host[i]= malloc(M * sizeof_v_val);
            AMGX_vector_download(x, result_host[i]);
        ...
        if (r > 0)
        {
            ...
            AMGX_vector_upload(x, n, block_dimx, x_h_repeat);
        }
    ...
    if (((pidx = findParamIndex(argv, argc, "-B")) != -1) && ((pidx = findParamIndex(argv, argc, "-X")) != -1)) {
        ...
        for (int i = 0; i < 3; i ++) {
            if (sizeof_v_val == sizeof(double)) {
                ...
                for (int j = 0; j < M; j++) {
                    error += fabs(result_host_d[j] - x_h_d[j]);
                }
            }
            else {
                float *result_host_f = (float *)result_host[i];
                float *x_h_f = (float *)x_h[i];

                for (int j = 0; j < M; j++)
                {
                    error += fabs(result_host_f[j] - x_h_f[j]);
                }
            }
